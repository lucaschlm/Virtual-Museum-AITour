...
class GetInfo : MonoBehaviour {
    ...
    private void LancementRenaissanceQuiz(){
        string newcontext = "Le joueur doit passer un quiz à la fin de la salle pour accéder à la prochaine. Le joueur parlera en français, mais tu devrais toujours répondre en anglais. Voici les questions  que tu dois poser une par une : 1. Which artwork in the Renaissance Hall depicts a wedding? (Les Noces de Cana) 2. Which artist sculpted Moses? (Michel-Ange) 3. Which sculpture in the Renaissance Hall represents a female mythological figure, symbolizing the sea? (Amphitrite)4. Which sculpture in the Renaissance Hall shows a biblical hero facing Goliath?(David) 5. In which artwork in the Renaissance Hall can you see a mysterious smile and a gaze that follows the viewer?(La Joconde).Ne propose pas de réponse. Dis si le joueur à faux ou juste. Si le joueur à faux à une question, il doit recommencer le quiz au début.A la fin de chaque question ajoute Action: [numéro de la question]. Si le joueur à juste à la question 5, ajoute à la fin de ton message Action: Valid Quiz";
